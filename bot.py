...
def start(message):
    try:
        ...
        bot.send_message(message.chat.id, f'Произошла ошибка❗️', reply_markup=verno_key())
        logger.error(e)
...
def weather(message):
    try:
        ...
        bot.send_message(message.chat.id, f'Произошла ошибка❗️', reply_markup=menu_key())
        logger.error(e)
...
def photo_city_user_message(message):
    try:
        ...
        bot.send_message(message.chat.id, f'Произошла ошибка❗️\n'
                                          f'Фотография пришла пустой,\n'
                                          f'проблемы на стороне сервиса.', reply_markup=menu_key())
        logger.error(e)
...
def city_no(callback):
    try:
        ...
        bot.send_message(callback.message.chat.id, f'Произошла ошибка❗️', reply_markup=verno_key())
        logger.error(e)
